...
1. `プロジェクト指示_v3.md` を読む — 工程0〜12・ゲート・出力順序のすべてがここにある
...
4. **起動ヘッダを出力する** — (a) 読み込んだ手順ファイル名＋版数、(b) 該当工程の全行チェックリストを空欄で先出し（`#予想`=工程0〜12・**冒頭で出走馬プロファイルの存在確認**（無ければ本題より先に `#収録` を提案して中断）、`#振り返り`=V0〜V7で**V4.5とV6.5を独立行**、`#収録`=種別がコース系ならC0〜C6・出走馬プロファイルなら『脚質認定ルール.md』§1〜§6の全頭実行、`#準備`=『週次運用スケジュール.md』第4項の9項目（**収録ギャップ表・`#収録` プロンプト・実行計画表を含む**。収録は検知と依頼文の生成までで実行しない））。「V0〜V7」等のまとめ宣言は禁止（中間工程の脱落を隠す）
...
- **買い目確定前（ゲート②）は `python3 log/gate.py` も実行する**。買い目系ルールの機械判定可能条文（R23/R24/R31/R32/R35/R36/R37）はスクリプトが判定し、LLMは裁量ルールの判定とFAIL時の理由記録（`欠落理由=` `軸選定=`）に専念する。GATE-FAILが残る買い目は確定しない。**`欠落理由=` は 2026-09-21 以降 R40 `消し根拠=` と同じ機械可読書式で書く**（`#<馬番>:cat=<閉じた語彙>:<事実タグ>:<自由文>`・項目は `;` 区切り。書いた事実タグ（妙味・final順位・本線）が predictions.csv と食い違うと validate.py が **ERROR**。書式は `log/README.md` が正本）
...
- **出走馬プロファイルは `python3 tools/build_profile.py --netkeiba-id <12桁> --slug <race_id> --name <レース名>` で生成する**（2026-09-11〜）。全頭の近5走・父母父・斤量・騎手・脚質機械認定・末脚指数・**`5走時計=`（時計/上がり順位/頭数/着差・v2.1／2026-09-20〜）**を1回で出す。LLM は生成物の差分レビュー（未確定馬・ハナ争い・不一致）と主観分類の確認だけを行い、馬1頭ずつ取得しない。必須列が欠けて止まったら「取得失敗」として報告する。過去走の単勝1倍台の走数は JSON にだけ保存して md には出さない（ブラインド評価）。**振り返りV6で `python3 tools/tag_odds1x.py --slug <race_id>` を実行し、`base_breakdown` に `1倍台=` を付ける**（仮説32・記録専用・2026-09-15〜）
